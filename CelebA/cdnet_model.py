...
class Encoder_Y(nn.Module):
    def __init__(self, channel_in=3, attr_size=40):
        ...
        for i in range(3):
            if i == 0:
                layers_list.append(EncoderBlock(channel_in=self.channel_num, channel_out=64))
                self.channel_num = 64
            else:
        ...
        self.fc = nn.Sequential(nn.Linear(in_features=8 * 8 * self.channel_num, out_features=4000, bias=False),
                                nn.LeakyReLU(0.2, inplace=True),
                                nn.Dropout(p=0.5),
                                nn.Linear(in_features=4000, out_features=2000, bias=False),
                                nn.LeakyReLU(0.2, inplace=True),
                                nn.Dropout(p=0.5),
                                nn.Linear(in_features=2000, out_features=attr_size))
...
class Encoder_Z(nn.Module):
    def __init__(self, channel_in=3, latent_size=1000):
...
class Decoder(nn.Module):
    def __init__(self, z_size=1040, channel_num=256):
        super(Decoder, self).__init__()

        self.fc1 = nn.Linear(z_size, 2000, bias=False)
        self.fc1_bn = nn.BatchNorm1d(2000)

        self.fc2 = nn.Linear(2040, 4000, bias=False)
        self.fc2_bn = nn.BatchNorm1d(4000)

        self.fc3 = nn.Linear(4040, 8 * 8 * channel_num, bias=False)
        self.fc3_bn = nn.BatchNorm1d(8 * 8 * channel_num)

        self.deconv1 = nn.ConvTranspose2d(channel_num + 40, channel_num // 2,
                                          kernel_size=4, stride=2, padding=1, bias=False)
        self.deconv1_bn = nn.BatchNorm2d(channel_num // 2)

        self.deconv2 = nn.ConvTranspose2d(channel_num // 2 + 40, channel_num // 4,
                                          kernel_size=4, stride=2, padding=1, bias=False)
        self.deconv2_bn = nn.BatchNorm2d(channel_num // 4)

        self.deconv3 = nn.ConvTranspose2d(channel_num // 4 + 40, 3,
                                          kernel_size=4, stride=2, padding=1)

    def forward(self, z_latent, y_attrs_sigmoid):
        z_y = torch.cat((z_latent, y_attrs_sigmoid), 1)
        z = F.relu(self.fc1_bn(self.fc1(z_y)), True)

        z_y = torch.cat((z, y_attrs_sigmoid), 1)
        z = F.relu(self.fc2_bn(self.fc2(z_y)), True)

        z_y = torch.cat((z, y_attrs_sigmoid), 1)
        z = F.relu(self.fc3_bn(self.fc3(z_y)), True)

        z = z.view(len(z), -1, 8, 8)
        y = y_attrs_sigmoid.unsqueeze(2).unsqueeze(3)
        y_feature_map = y.expand(y.size(0), 40, z.size(2), z.size(2))
        z_y = torch.cat((z, y_feature_map), 1)
        z = F.relu(self.deconv1_bn(self.deconv1(z_y)), True)

        y_feature_map = y.expand(y.size(0), 40, z.size(2), z.size(2))
        z_y = torch.cat((z, y_feature_map), 1)
        z = F.relu(self.deconv2_bn(self.deconv2(z_y)), True)

        y_feature_map = y.expand(y.size(0), 40, z.size(2), z.size(2))
        z_y = torch.cat((z, y_feature_map), 1)
        x = torch.tanh(self.deconv3(z_y))
        return x
